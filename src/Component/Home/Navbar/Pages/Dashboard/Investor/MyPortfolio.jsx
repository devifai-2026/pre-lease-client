...
const MyPortfolio = () => {
  ...
  // Lease Renewals Data
  const leaseRenewals = [
    { id: 1, property: 'Residential Space', location: 'Pune', tenant: 'AP Realtors', expiryDate: '15/12/2025', annualRent: '₹2,65,00,000', status: 'Expiring Soon' },
    { id: 2, property: 'Commercial Space', location: 'Mumbai', tenant: 'Global Innovations', expiryDate: '20/12/2025', annualRent: '₹2,55,00,000', status: 'Expiring Soon' }
  ];
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 font-montserrat">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 mb-6">
        ...
        <div className="bg-white shadow-lg rounded-lg p-4">
          <h3 className="text-base sm:text-lg font-semibold text-[#262626] mb-6 text-center">Portfolio Diversification</h3>
          ...
          <div className="flex justify-center gap-6 mt-4 flex-wrap">
            {diversificationData.map((item, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <span className="text-xs sm:text-sm text-[#767676]">{item.name} {item.value}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Income Tracker */}
        <div className="bg-white shadow-lg rounded-lg p-4">
          <h3 className="text-base sm:text-lg font-semibold text-[#262626] mb-6 text-center">Income Tracker</h3>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={incomeTrackerData}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="received" stroke="#EE2529" strokeWidth={2} name="Received" />
              <Line type="monotone" dataKey="expected" stroke="#767676" strokeWidth={2} strokeDasharray="5 5" name="Expected" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-6 mb-6">
        <div className="flex flex-col md:flex-row space-y-2 justify-between items-center mb-4">
          <h3 className=" text-lg md:text-xl lg:text-2xl font-semibold text-[#262626]">Upcoming Lease Renewals</h3>
          <span className="text-sm text-[#EE2529] font-semibold bg-[#FDEDEE] rounded-3xl py-1 px-2">{leaseRenewals.length} Expiring Soon</span>
        </div>
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-[#E0E0E0]">
                ...
                <th className="text-left py-3 px-2 font-semibold text-[#262626]">Tenant</th>
                <th className="text-left py-3 px-2 font-semibold text-[#262626]">Expiry Date</th>
            ...
            <tbody>
              {leaseRenewals.map((renewal) => (
                <tr key={renewal.id} className="border-b border-[#E0E0E0]">
                  <td className="py-3 px-2 text-[#262626]">{renewal.property}</td>
                  <td className="py-3 px-2 text-[#767676]">{renewal.location}</td>
                  <td className="py-3 px-2 text-[#767676]">{renewal.tenant}</td>
                  <td className="py-3 px-2 text-[#262626]">{renewal.expiryDate}</td>
                  <td className="py-3 px-2 text-[#262626]">{renewal.annualRent}</td>
      ...
      <div className="mb-6">
        <div className="flex flex-col md:flex-row space-y-2 justify-between items-center mb-4">
          <h3 className="text-lg md:text-xl lg:text-2xl font-semibold text-[#EE2529]">Properties Owned</h3>
          <div className="flex gap-3">
            <select value={filterDays} onChange={(e) => setFilterDays(e.target.value)} className="text-xs sm:text-sm border border-[#E0E0E0] rounded px-2 py-1">
              <option value="30">Last 30 Days</option>
              <option value="60">Last 60 Days</option>
              <option value="90">Last 90 Days</option>
            </select>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="text-xs sm:text-sm border border-[#E0E0E0] rounded px-2 py-1">
