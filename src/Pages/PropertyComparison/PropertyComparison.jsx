...
const handleDownload = async () => {
    ...
    try {
        ...
        // Now process the content for PDF
        // Fix table widths and styling for PDF
        ...
        // Fix overflow-x-auto containers
        const overflowContainers = tempContainer.querySelectorAll('[style*="overflow-x"]');
        overflowContainers.forEach(container => {
            container.style.overflowX = 'visible';
            container.style.width = '100%';
        });
        
        // Fix other grid layouts
        const grids = tempContainer.querySelectorAll('[class*="grid"]');
        grids.forEach(grid => {
            // Skip property cards grid since we handled it separately
            if (!grid.className.includes('grid-cols-3') && 
                !grid.textContent.includes('Residential Space') &&
                !grid.textContent.includes('Bank Space') &&
                !grid.textContent.includes('Retail Hub')) {
        ...
        // Fix icon alignment issues for other components
        const locationDivs = tempContainer.querySelectorAll('.flex.items-center');
        locationDivs.forEach(div => {
            if (div.textContent && div.textContent.includes('CiLocationOn') || 
                div.innerHTML.includes('CiLocationOn')) {
        ...
        // Fix client type badge and buttons vertical alignment in other components
        const badges = tempContainer.querySelectorAll('.bg-\\[\\#FFF3CA\\]');
        ...
        const shareIcons = tempContainer.querySelectorAll('.bg-\\[\\#2626268A\\]');
        ...
        if (finalImgHeight > pdfHeight) {
            ...
            while (remainingHeight > 0) {
                ...
                pageCanvas.height = Math.min(
                    remainingHeight,
                    (pdfHeight / finalImgHeight) * imgHeight
                );
                ...
                pdf.addImage(pageImgData, 'PNG', 0, 0, finalImgWidth, pdfHeight);
                
                remainingHeight -= pageCanvas.height;
                currentYPosition += pageCanvas.height;
            }
        } else {
