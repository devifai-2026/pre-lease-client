...
const AnnualRent = ({ onRentChange, initialRent }) => {
  ...
  const handleInputChange = (e, type) => {
    ...
    if (type === 'min') {
      const lakhValue = Math.min(Math.max(Math.floor(value / 100000), 0), 49); // Max 49 to leave room for max
      setMinRent(value);
      setRentRange([lakhValue, rentRange[1]]);
      onRentChange?.({ min: value, max: maxRent });
    } else {
      const lakhValue = Math.min(Math.max(Math.floor(value / 100000), 1), 50); // Min 1 to leave room for min
      setMaxRent(value);
